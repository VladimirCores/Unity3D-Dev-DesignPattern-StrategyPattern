...
public class SimpleTweener
{
    ...
    public SimpleTweener(float startValue, float endValue, float time, EaseType ease) : this(startValue, endValue, time, 0, ease)
    {}

    public SimpleTweener(float startValue, float endValue, float time, float delay, EaseType ease)
    {
    ...
    {
        bool isavailable = SetTime(CurrentTime + deltaTime);
        return isavailable;
    }
    public bool UpdateTime(float deltaTime, ref float value)
    {
        bool isavailable = SetTime(CurrentTime + deltaTime);
        value = CurrentValue;
        return isavailable;
    ...
    public bool SetTime(float time)
    {
        ...
        CurrentValue = ease(StartValue, EndValue, CurrentTime >= 0 ? CurrentTime*invTime : 0);
        return false;
    }
    ...
    static private float easeInBounce(float start, float end, float value)
    {
        ...
        float d = 1f;
        return end - SimpleTweener.easeOutBounce(0, end, d - value) + start;
    }
